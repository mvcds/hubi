...
* `--same-folder | -s` overides the output flag, by geneating the source file on the same folder as the token's domain file. Some translators may not work with it, e.g. the site
